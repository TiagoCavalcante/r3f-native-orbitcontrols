...
function createControls() {
  ...
  var functions = {
    ...
    trackPointer: function (event) {
      var position = internals.pointerPositions[event.pointerId]
    ...
    getSecondPointerPosition: function (event) {
      var pointer = event.pointerId === internals.pointers[0].pointerId
    ...
    handleTouchStartRotate: function () {
      if (internals.pointers.length == 1) {
        internals.rotateStart.set(internals.pointers[0].pageX, internals.pointers[0].pageY)
      }
      else {
        var x = 0.5 * (internals.pointers[0].pageX + internals.pointers[1].pageX)
        var y = 0.5 * (internals.pointers[0].pageY + internals.pointers[1].pageY)
        internals.rotateStart.set(x, y)
      }
    },
    handleTouchStartDolly: function () {
      var dx = internals.pointers[0].pageX - internals.pointers[1].pageX
      var dy = internals.pointers[0].pageY - internals.pointers[1].pageY
      var distance = Math.sqrt(dx * dx + dy * dy)
      internals.dollyStart.set(0, distance)
    },
    handleTouchStartPan: function () {
      if (internals.pointers.length === 1) {
        internals.panStart.set(internals.pointers[0].pageX, internals.pointers[0].pageY)
      }
      else {
        var x = 0.5 * (internals.pointers[0].pageX + internals.pointers[1].pageX)
        var y = 0.5 * (internals.pointers[0].pageY + internals.pointers[1].pageY)
    ...
    handleTouchMoveRotate: function (event) {
      ...
      // yes, height
      this.rotateLeft((2 * Math.PI * internals.rotateDelta.x) / reactNative.Dimensions.get('window').height)
      this.rotateUp((2 * Math.PI * internals.rotateDelta.y) / reactNative.Dimensions.get('window').height)
    ...
    pan: function (deltaX, deltaY) {
      ...
      this.panLeft((2 * deltaX * targetDistance) / reactNative.Dimensions.get('window').height, scope.camera.matrix)
      this.panUp((2 * deltaY * targetDistance) / reactNative.Dimensions.get('window').height, scope.camera.matrix)
    },
  ...
  var update = (function () {
    ...
    var quat = new three.Quaternion()
      .setFromUnitVectors(scope.camera.up, new three.Vector3(0, 1, 0))
...
function OrbitControls(_a) {
  ...
  React.useEffect(function () {
    for (var prop in props) {
      controls.scope[prop] = props[prop]
    }
  }, [props])
  fiber.useFrame(controls.functions.update, -1)
  return null
}
function useControls() {
  var controls = createControls()
  return [
    function (props) { return React__default["default"].createElement(OrbitControls, __assign({ controls: controls }, props)) },
    controls.events
  ]
}
