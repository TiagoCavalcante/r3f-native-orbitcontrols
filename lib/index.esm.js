...
import { Vector3, Vector2, Spherical, Quaternion } from 'three';
...
function useCreateControls(mode) {
    var _a = useState(0), height = _a[0], setHeight = _a[1];
    var STATE = mode === "map" /* ControlsMode.MAP */
        ? {
            NONE: 0,
            PAN: 1,
            ROTATE_OR_ZOOM: 2,
    ...
    var scope = useState(function () { return (__assign(__assign({}, partialScope), { mode: mode, target: new Vector3(), onChange: function (event) { } })); })[0];
    var internals = {
    ...
    var functions = {
        shouldClaimTouch: function (event) {
            ...
            if (event.nativeEvent.touches.length === 1) {
                var _a = event.nativeEvent.touches[0], x = _a.pageX, y = _a.pageY, t = _a.timestamp;
        ...
        handleTouchStartRotate: function (event) {
            if (event.nativeEvent.touches.length === 1) {
                internals.rotateStart.set(event.nativeEvent.touches[0].pageX, event.nativeEvent.touches[0].pageY);
            }
        ...
        handleTouchStartDolly: function (event) {
            ...
            if (event.nativeEvent.touches.length === 2) {
                var dx = event.nativeEvent.touches[0].pageX -
                    event.nativeEvent.touches[1].pageX;
                var dy = event.nativeEvent.touches[0].pageY -
                    event.nativeEvent.touches[1].pageY;
        ...
        handleTouchStartPan: function (event) {
            if (event.nativeEvent.touches.length === 1) {
                internals.panStart.set(event.nativeEvent.touches[0].pageX, event.nativeEvent.touches[0].pageY);
            }
            else if (event.nativeEvent.touches.length === 2) {
                var x = 0.5 *
                    (event.nativeEvent.touches[0].pageX +
        ...
        onTouchStart: function (event) {
            if (!scope.enabled)
                return;
            if (scope.mode === "map" /* ControlsMode.MAP */) {
        ...
        handleTouchMoveRotate: function (event) {
            if (event.nativeEvent.touches.length === 1) {
                internals.rotateEnd.set(event.nativeEvent.pageX, event.nativeEvent.pageY);
            }
            else if (event.nativeEvent.touches.length === 2) {
                var x = 0.5 *
                    (event.nativeEvent.touches[0].pageX +
        ...
        onTouchMove: function (event) {
            ...
            switch (internals.state) {
                case STATE.ROTATE:
                    if (scope.mode === "orbit" /* ControlsMode.ORBIT */)
                        return;
                    if (!scope.enableRotate)
                        return;
        ...
        handleTouchStartRotateOrZoom: function (event) {
            if (event.nativeEvent.touches.length !== 2)
                return;
            var touch1 = event.nativeEvent.touches[0];
            var touch2 = event.nativeEvent.touches[1];
            ...
            var midX = (touch1.pageX + touch2.pageX) / 2;
            var midY = (touch1.pageY + touch2.pageY) / 2;
            internals.rotateStart.set(midX, midY);
        },
        handleTouchMoveRotateOrZoom: function (event) {
            if (event.nativeEvent.touches.length !== 2)
                return;
            var touch1 = event.nativeEvent.touches[0];
            var touch2 = event.nativeEvent.touches[1];
            ...
            var angleDelta = getShortestAngle(internals.initialRotation, currentAngle);
            var midX = (touch1.pageX + touch2.pageX) / 2;
            var midY = (touch1.pageY + touch2.pageY) / 2;
            var deltaY = midY - internals.rotateStart.y;
            var timeDelta = event.nativeEvent.timestamp - internals.lastMoveTimestamp;
            var distanceDelta = Math.abs(distance - internals.initialDistance);
            var zoomSpeed = distanceDelta / timeDelta;
            if (zoomSpeed > ZOOM_SPEED_THRESHOLD && scope.enableZoom) {
                // Handle zoom
                var scale = distance / internals.initialDistance;
            ...
            if (Math.abs(deltaY) > 2) {
                var tiltSpeed = 0.01 * scope.rotateSpeed;
...
function Controls(_a) {
    ...
    useEffect(function () {
        if (camera.isPerspectiveCamera ||
            ...
            throw new Error("The camera must be a PerspectiveCamera or OrthographicCamera for controls to work");
        }
...
function useControls(mode) {
    if (mode === void 0) { mode = "orbit" /* ControlsMode.ORBIT */; }
    var controls = useMemo(function () { return useCreateControls(mode); }, [mode]);
    return [
        function (props) { return React.createElement(Controls, __assign({ controls: controls }, props)); },
        controls.events,
    ];
}
